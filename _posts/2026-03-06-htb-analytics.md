...
image: 
    path: Analytics.webp
    lqip: data:image/webp;base64,UklGRlIAAABXRUJQVlA4IEYAAABwAwCdASoUAAsAPzmGuVOvKSWisAgB4CcJZQDE2BjYSe9zP9gA/ujxEdGodU9k5qgcFJhGkZOF3mdF4f85pZd15NHv9QAA
    alt: Info Card
---

## Summary

[Analytics](https://www.hackthebox.com/machines/analytics){: target="_blank" } is an easy-difficulty Linux machine featuring a vulnerable Metabase instance. We leverage a pre-authentication RCE (CVE-2023-38646) to gain an initial foothold inside a Docker container. From there, we extract hardcoded credentials from environment variables to SSH into the host. Finally, we achieve root access by exploiting a vulnerable kernel version.
...
![Add domain](hosts1.webp)
_Domain added to /etc/hosts_

Looking at the website, it seems to be a static page but the login tab redirects to a subdomain: **data.analytical.htb**

![Website](web.webp)
_Web Page_

![Subdomain](subdomain.webp)
_Subdomain data.analytical.htb_

To access that site we have to add the subdoaim to our `/etc/hosts`{: .filepath} file:

![Add Subdomain](hosts2.webp)
_Subdomain added to /etc/hosts_
...
![Metabase Login Panel](loginpanel.webp)
...
![Searching for Metabase Vulnerabilities](searchmeta.webp)
_Vulnerabilities search_
...
![Downloading Exploit](exploit.webp)
_Downloading exploit_
...
![Credentials](creds.webp)
_Credentials stored in environment variables_
...
_Successful SSH login to the host_
...
![System Info](uname.webp)
_System Info_

![Privilege Escalation Search](searchpriv.webp)
_Kernel Exploit search_
...
![Privilege Escalation](privesc.webp)
_Shell as Root_
